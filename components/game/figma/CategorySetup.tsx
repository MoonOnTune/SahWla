...
import { GAME_POINT_VALUE_SLOTS } from '@/lib/game/catalog';
...
export function CategorySetup() {
  const { setScreen, setCategories, questionBankByCategory } = useGame();
  ...
  const handleNext = () => {
    ...
    const cats: Category[] = selected.map((i) => {
      const categoryMeta = PRESET_CATEGORIES[i];
      const questionPool = questionBankByCategory[categoryMeta.name] ?? [];

      if (questionPool.length !== GAME_POINT_VALUE_SLOTS.length) {
        throw new Error(`MISSING_SESSION_PICKS:${categoryMeta.name}`);
      }

      return {
        name: categoryMeta.name,
        type: categoryMeta.type,
        questions: questionPool.map((item) => ({
      ...
      <motion.button
          ...
          onClick={() => {
            ...
            try {
              ...
              if (error instanceof Error && error.message.startsWith('MISSING_SESSION_PICKS:')) {
                const [, categoryName] = error.message.split(':');
                setSelectionError(`لا توجد أسئلة جلسة جاهزة للفئة: ${categoryName}`);
              } else {
                setSelectionError('تعذر تجهيز الفئات المختارة. ابدأ جلسة جديدة.');
              }
            }
          }}
        ...
        {selectionError && (
          <p className="text-red-400 text-sm mt-3" style={{ fontFamily: 'Cairo, sans-serif' }}>
            {selectionError}
          </p>
        )}
      </div>
