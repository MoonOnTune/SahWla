...
import { sendRoomChatMessage } from "@/lib/game/room-service";
import { sendRoomChatSchema } from "@/lib/validation/api";
...
export async function POST(request: Request, context: { params: Promise<{ roomCode: string }> }) {
  ...
  const parsed = sendRoomChatSchema.safeParse({
    roomCode,
  ...
  try {
    const snapshot = await sendRoomChatMessage({
      roomCode,
      deviceToken,
